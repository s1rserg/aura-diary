...
const signIn = createAsyncThunk<ApiAuthPayload, SignInCredentials, AsyncThunkConfig>(
  `${name}/signIn`,
...
const signUp = createAsyncThunk<ApiAuthPayload, SignUpCredentials, AsyncThunkConfig>(
  `${name}/signUp`,
  async (credentials, { extra }) => {
    const { authService } = extra;
